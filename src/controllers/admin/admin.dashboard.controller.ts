...
export class AdminDashboardController{
    constructor(@inject("IAdminDashboardService") private readonly _adminDashboardService:IAdminDashboardService){}

    async getDashboard(req:Request,res:Response,next:NextFunction){
        try {
            const data = await this._adminDashboardService.getDashboardData();
            return res.status(STATUS_CODES.SUCCESS).json({success:true,message:MESSAGES.DATA_FETCH_SUCCESS,data})
        } catch (error) {
