...
export class StripeWebhookController{
  constructor(@inject("IEmiPaymentService") private _iEmiPaymentService:IEmiPaymentService,
  ...
  async handleWebhook(req:Request,res:Response,next:NextFunction){
    try {
      ...
      if(!signature || typeof signature !== "string"){
        throw new CustomError(MESSAGES.STRIPE_SIGNATURE_MISSING,STATUS_CODES.BAD_REQUEST)
      }

      const event = await this._iStripeService.constructWebhookEvent(req.body as Buffer,signature);
      ...
      res.status(STATUS_CODES.SUCCESS).json({success:true,message:MESSAGES.WEBHOOK_RECIEVED})
