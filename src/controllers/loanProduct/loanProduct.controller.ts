...
export class LoanProductController {
  constructor(
    @inject("ILoanProductService")
    private _iloanProductService: ILoanProductService,
  ) {}
  ...
  async createLoanProduct(
    ...
    try {
      ...
      const response = await this._iloanProductService.createLoanProduct(
        data,
        vendorId.toString(),
      );
      ...
      res
        ...
        .json({ success: true, message: MESSAGES.SUCCESS, data: response });
    } catch (error) {
      next(error);
    }
  }
  ...
  async getVendorLoans(
    ...
    try {
      ...
      const loans = await this._iloanProductService.getLoansByVendor(
        vendorId,
        search,
        page,
        limit,
      ...
      res
        ...
        .json({ success: true, message: MESSAGES.SUCCESS, data: loans });
    } catch (error) {
      next(error);
    }
  }
  ...
  async updateLoanByVendor(
    ...
    try {
      ...
      const updatedLoan = await this._iloanProductService.updateLoanByVendor(
        loanId,
        vendorId,
        updateData,
      );
      ...
      return res
        ...
        .json({
          success: true,
          message: MESSAGES.LOAN_UPDATED_SUCCESSFULLY,
          data: updatedLoan,
        });
    } catch (error) {
      next(error);
  ...
  async getLoanById(
    ...
    try {
      ...
      const loan = await this._iloanProductService.getLoanIdByVendor(
        loanId,
        vendorId,
      );
  ...
  async getLoanDetails(
    ...
    try {
      ...
      if (!loanId) {
        throw new CustomError(
          MESSAGES.LOAN_ID_REQUIRED,
          STATUS_CODES.BAD_REQUEST,
        );
      }

      const loanDetails = await this._iloanProductService.getLoanDetails(
        loanId,
        vendorId,
      );
      ...
      return res
        ...
        .json({
          success: true,
          message: MESSAGES.LOAN_DETAILS_FETCHED,
          data: loanDetails,
        });
    } catch (error) {
      next(error);
  ...
  async getLoanDetailsForUser(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const loan =
        await this._iloanProductService.getLoanDetailsForUsers(loanId);
