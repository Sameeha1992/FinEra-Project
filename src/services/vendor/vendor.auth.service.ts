...
export class VendorAuthService implements IVendorAuthService {
  ...
  async verifyOtp(vendorData: OtpVerifyDto): Promise<boolean> {
    try {
      ...
      if (!otp || otp.length !== 6) {
        throw new CustomError(MESSAGES.OTP_INVALID);
      }
  ...
  async vendorLogin(
    ...
    const isPassword: boolean = await this._IpasswordService.comparePassword(
  ...
  async refreshToken(
    ...
    if (ttlSeconds > 0) {
      await this._redisService.blacklistRefreshToken(decode.jti, ttlSeconds);
    }
  ...
  async googleLogin(
    ...
    let vendor: IVendor | null =
      await this._vendorRepository.findByEmail(email);
